...
export const getAipSummaryColumnDefs = <T extends Record<string, any>>(): ColumnDef<T>[] => [
    {
        id: "ref_code",
        width: AIP_SUMMARY_COLUMN_WIDTHS[0],
        cell: (item) => <Text style={cellStyle("center")}>{item.ref_code || ""}</Text>,
    },
    {
        id: "description",
        width: AIP_SUMMARY_COLUMN_WIDTHS[1],
        cell: (item) => <Text style={cellStyle("left")}>{item.label || ""}</Text>,
    },
    {
        id: "implementing_office",
        width: AIP_SUMMARY_COLUMN_WIDTHS[2],
        cell: (item) => <Text style={cellStyle("center")}>{item.officeAcronyms || ""}</Text>,
    },
    {
        id: "start_date",
    ...
    },
    {
        ...
        cell: (item) => <Text style={cellStyle("center")}>{item.expected_output || ""}</Text>,
    },
    {
        id: "funding_source",
        width: AIP_SUMMARY_COLUMN_WIDTHS[6],
        cell: (item) => <Text style={cellStyle("center")}>{item.funding_source_code || "-"}</Text>,
    },
    {
        id: "ps",
        width: AIP_SUMMARY_COLUMN_WIDTHS[7],
        cell: (item) => <Text style={cellStyle("right")}>{formatCurrency(item.ps || 0)}</Text>,
    },
    {
        id: "mooe",
        width: AIP_SUMMARY_COLUMN_WIDTHS[8],
        cell: (item) => <Text style={cellStyle("right")}>{formatCurrency(item.mooe || 0)}</Text>,
    },
    {
        id: "fe",
        width: AIP_SUMMARY_COLUMN_WIDTHS[9],
        cell: (item) => <Text style={cellStyle("right")}>{formatCurrency(item.fe || 0)}</Text>,
    },
    {
        id: "co",
        width: AIP_SUMMARY_COLUMN_WIDTHS[10],
        cell: (item) => <Text style={cellStyle("right")}>{formatCurrency(item.co || 0)}</Text>,
    },
    {
        id: "total",
    ...
    },
    {
        ...
        cell: (item) => (
            <Text style={cellStyle("right")}>{formatCurrency(item.ccet_adapt || 0)}</Text>
        ),
    },
    {
        id: "ccet_miti",
        ...
        cell: (item) => (
            <Text style={cellStyle("right")}>{formatCurrency(item.ccet_miti || 0)}</Text>
        ),
    },
    {
        id: "cc_typology",
        width: AIP_SUMMARY_COLUMN_WIDTHS[14],
        cell: (item) => <Text style={cellStyle("center")}>{item.cc_typology_code || "-"}</Text>,
    },
];
