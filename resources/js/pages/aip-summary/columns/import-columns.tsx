...
const columns = [
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {isAdded && (
                            <Badge variant="outline" className="h-4 px-1 text-[9px]">
                                <CheckCircle2 className="mr-1 h-2 w-2" /> IN SUMMARY
                            </Badge>
                        )}
                    </div>
    ...
    columnHelper.display({
        ...
        cell: ({ row, table }) => {
            const meta = table.options.meta as any;
            const ppa = row.original;
            const ppaTypes = meta?.ppaTypes as string[] | undefined;
            const isLeaf = ppaTypes ? ppaTypes.indexOf(ppa.type) === ppaTypes.length - 1 : false;
            const canOpen = !isLeaf;
