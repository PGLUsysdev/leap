...
export default function getColumns(
    ...
    const columns = [
        columnHelper.accessor("item_number", {
            ...
            cell: (info) => (
                <div className="px-1 text-wrap">{info.row.original.ios?.class ?? "—"}</div>
            ),
        ...
        columnHelper.accessor("user", {
            ...
            cell: (info) => (
                <div className="px-1 text-wrap">{info.getValue()?.name ?? "Vacant"}</div>
            ),
        }),
        ...
        columnHelper.display({
            ...
            cell: ({ row }) => {
                ...
                return <div className="px-1 text-right text-wrap">{currency(monthly)}</div>;
            },
            footer: ({ table }) => {
        ...
        columnHelper.display({
            ...
            cell: ({ row }) => {
                ...
                return <div className="px-1 text-right text-wrap">{currency(annual)}</div>;
            },
            footer: ({ table }) => {
                const total = table.getCoreRowModel().rows.reduce((sum, row) => {
        ...
        ...coas.map((coa) =>
            columnHelper.display({
                id: `coa_${coa.id}`,
                size: 310,
                header: () => <div className="px-1 text-right">{coa.account_title}</div>,
                cell: ({ row }) => {
                    ...
                    return value !== null ? (
                        <div className="px-1 text-right text-wrap">{currency(value)}</div>
                    ) : (
