...
const canNavigateInto = (targetType: string, sourceType: string): boolean => {
    ...
    switch (sourceType) {
        ...
        case "Sub-Activity":
            ...
            return (
                targetType === "Program" || targetType === "Project" || targetType === "Activity"
            );
...
const columns = [
    columnHelper.display({
        ...
        cell: ({ row, table }) => {
            ...
            if (!source) return <div className="h-4 w-4 rounded border border-dashed opacity-20" />;
    ...
    columnHelper.accessor("full_code", {
        ...
        cell: (info) => {
            ...
            const isSelf = String(info.row.original.id) === String(meta.ppaToMove?.id);
            return (
                <code className={`font-mono text-xs ${isSelf ? "opacity-30" : ""}`}>
            ...
            return (
                ...
                >
                    <div className="mb-1 flex items-center gap-2">
                        ...
                        {isSibling && (
                            <Badge className="h-4 px-1 text-[9px] font-bold">SIBLING</Badge>
                        )}
                        {isSelf && (
    ...
    columnHelper.display({
        ...
        cell: ({ row, table }) => {
            ...
            const isNavigationAllowed = source ? canNavigateInto(target.type, source.type) : false;
