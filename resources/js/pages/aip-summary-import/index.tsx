...
import type { AipSummaryField, AipSummarySheetConfig } from "@/lib/aip-summary-import/sheet-config";
import {
...
import { extractAipSummaryRows, verifyAipSummarySheet } from "@/lib/aip-summary-import/verify";
...
export default function AipSummaryImport() {
    ...
    const [step, setStep] = useState<"upload" | "calibrate" | "verify">("upload");
    const [config, setConfig] = useState<AipSummarySheetConfig>(() => getDefaultAipSummaryConfig());
    const [verifyResult, setVerifyResult] = useState<AipSummaryVerifyResult | null>(null);

    const canCalibrate = selectedSheet !== "";
    const canVerify =
        canCalibrate && config.headerRow !== "" && config.headerRow != null && !!workbook;
    ...
    function updateColumn(field: AipSummaryField, letter: string) {
        setConfig((prev) => ({
            ...prev,
            columnConfig: { ...prev.columnConfig, [field]: letter.toUpperCase() },
        }));
        setVerifyResult(null);
    }
    ...
    function handleLogContents() {
        ...
        if (config.headerRow === "" || config.headerRow == null) {
            console.log("AipSummaryImport: header row is required before logging contents.");
        ...
        if (!ws) {
            console.log(`AipSummaryImport: worksheet "${selectedSheet}" not found.`);
    ...
    return (
        <>
            ...
            <div className="flex flex-col gap-4 p-4 md:p-6">
                <h1 className="text-2xl font-semibold tracking-tight">AIP Summary Import</h1>
                ...
                <Tabs value={step} onValueChange={(v) => setStep(v as typeof step)}>
                    ...
                    <TabsContent value="upload" className="mt-4 flex flex-col gap-4">
                        ...
                        <div className="flex justify-end">
                            <Button disabled={!canCalibrate} onClick={() => setStep("calibrate")}>
                                Next: Calibrate
                            </Button>
                        </div>
                    </TabsContent>
                    ...
                    <TabsContent value="calibrate" className="mt-4 flex flex-col gap-4">
                        <Field>
                            <FieldLabel htmlFor="aip-summary-header-row">Header row</FieldLabel>
                            <Input
                            ...
                            />
                            <FieldDescription>
                                1-indexed leaf-header row. The number row (`1`–`15`) is always one
                                row below
                                    ...
                                    {group.fields.map((field) => (
                                        <div key={field} className="flex items-center gap-2">
                                            <label
                                                ...
                                                className="w-36 shrink-0 text-sm text-muted-foreground"
                                            >
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2">
                            ...
                            <div className="flex gap-2">
                                <Button onClick={handleLogContents} disabled={!selectedSheet}>
                                    <ScrollText className="h-4 w-4" /> Log contents
                                </Button>
                                <Button disabled={!canVerify} onClick={() => setStep("verify")}>
                                    Next: Verify
                                </Button>
                            </div>
                        </div>
                    ...
                    <TabsContent value="verify" className="mt-4 flex flex-col gap-4">
                        <p className="text-sm text-muted-foreground">
                            Sheet{" "}
                            <span className="font-medium text-foreground">{selectedSheet}</span> ·
