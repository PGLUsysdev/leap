...
export function UploadStep({ s }: { s: CategoryImportState }) {
    ...
    function handleSheetsChange(next: unknown) {
        console.log('[category-import handleSheetsChange] raw next:', next, 'selectedSheets before:', selectedSheets);
        const flat = (Array.isArray(next) ? (next as unknown[]).flat(Infinity) : [])
            .map((s) => String(s).trim())
            .filter(Boolean) as string[];
        console.log('[category-import handleSheetsChange] flat:', flat);
        const picked = flat[0] ?? '';
