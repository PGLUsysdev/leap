...
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/base-ui-components/ui/table";
...
type ExtractResult = {
    ...
    duplicates: Array<{ normalized: string; keptRow: number; duplicateRow: number; duplicateRaw: string }>;
    excludedTotal: Array<{ row: number; raw: string; normalized: string }>;
    excludedCoa: Array<{ row: number; raw: string; normalized: string; nextRowCoaRaw: string; nextRowCoaNormalized: string }>;
    skippedCoaNotEmpty: Array<{ row: number; coaRaw: string; coaNormalized: string; raw: string; normalized: string }>;
...
export default function CategoryImport() {
    ...
    function handleExtract() {
        ...
        const excludedCoa: Array<{ row: number; raw: string; normalized: string; nextRowCoaRaw: string; nextRowCoaNormalized: string }> = [];
        const skippedCoaNotEmpty: Array<{ row: number; coaRaw: string; coaNormalized: string; raw: string; normalized: string }> = [];
        const skippedProblematic: Array<{ row: number; raw: string; normalized: string; reason: string }> = [];
        ...
        for (let r = startRow; r <= lastRow; r++) {
            ...
            if (skipProblematic && verifyResult && !verifyResult.valid) {
                if (problematicRows.has(r) || problematicNorms.has(dataNorm)) {
                    const reason = problematicRows.has(r) ? `row ${r} flagged in verify` : `normalized "${dataNorm}" flagged`;
                ...
                if (coaNorm && (problematicRows.has(r) || problematicNorms.has(coaNorm))) {
                    skippedProblematic.push({ row: r, raw: dataRaw, normalized: dataNorm, reason: `COA "${coaRaw}" flagged` });
                    continue;
                }
            }

            if (coaNorm) {
                skippedCoaNotEmpty.push({ row: r, coaRaw: coaRaw!, coaNormalized: coaNorm, raw: dataRaw, normalized: dataNorm });
                continue;
            }
            if (isTotalRow(dataNorm)) {
            ...
            if (r + 1 <= lastRow) {
                ...
                if (nextCoaNorm && nextCoaNorm === dataNorm) {
                    excludedCoa.push({ row: r, raw: dataRaw, normalized: dataNorm, nextRowCoaRaw: nextCoaRaw!, nextRowCoaNormalized: nextCoaNorm });
                    continue;
                }
            }
        ...
        const duplicates: Array<{ normalized: string; keptRow: number; duplicateRow: number; duplicateRaw: string }> = [];
        for (const c of filtered) {
            const existing = seen.get(c.normalized);
            if (!existing) seen.set(c.normalized, { raw: c.raw, normalized: c.normalized, rows: [c.row] });
            else {
                existing.rows.push(c.row);
                duplicates.push({ normalized: c.normalized, keptRow: existing.rows[0], duplicateRow: c.row, duplicateRaw: c.raw });
            }
        }
        const unique = [...seen.values()].map((v) => ({ ...v, count: v.rows.length }));
        console.log(filtered, unique, duplicates);
        if (skippedProblematic.length > 0) console.table(skippedProblematic.map((s) => ({ row: s.row, raw: s.raw, normalized: s.normalized, reason: s.reason, status: "skipped: problematic" })));
    ...
    function handleImport() {
        ...
        router.post(
            ...
            { categories: extractResult.unique.map((u) => ({ name: u.raw, normalized: u.normalized })) } as never,
            {
                ...
                <Tabs value={step} onValueChange={(v) => setStep(v as typeof step)}>
                    <TabsList variant="line" className="w-full">
                        <TabsTrigger value="upload" className="flex-1">
                            1. Upload & Sheet
                            {selectedSheet && <span className="ml-1 text-xs text-muted-foreground">✓</span>}
                        </TabsTrigger>
                        ...
                        <TabsTrigger value="verify" disabled={!canVerify}>
                            3. Verify Format
                            {verifyResult?.valid && <span className="ml-1 text-xs text-green-600">✓</span>}
                        </TabsTrigger>
                        <TabsTrigger value="extract" disabled={!canExtract}>
                            4. Extract & Import
                            {extractResult && <span className="ml-1 text-xs text-muted-foreground">{extractResult.unique.length}</span>}
                            {!verifyResult?.valid && skipProblematic && <span className="ml-1 text-xs text-amber-600">skip</span>}
                        </TabsTrigger>
                    </TabsList>
                    ...
                    <TabsContent value="upload" className="mt-4 flex flex-col gap-4">
                        <Field>
                            ...
                            <Input
                                ...
                                disabled={loading}
                            />
                            <FieldDescription>Select an .xlsx file. Only .xlsx is accepted (ExcelJS).</FieldDescription>
                        ...
                        {!loading && fileName && (
                            <div className="text-sm text-muted-foreground">
                                File: <span className="font-medium text-foreground">{fileName}</span> — {sheets.length} sheet{sheets.length === 1 ? "" : "s"} found
                            </div>
                        )}
                        {!loading && sheets.length > 0 && (
                            <Field>
                                ...
                                <FieldDescription>Selected: <span className="font-medium text-foreground">{selectedSheet ?? "none"}</span></FieldDescription>
                            </Field>
                        )}
                    ...
                    <TabsContent value="calibrate" className="mt-4 flex flex-col gap-4">
                        <div className="rounded-lg border p-4">
                            <p className="mb-3 text-xs font-semibold tracking-wide text-muted-foreground uppercase">Calibration</p>
                            <div className="grid grid-cols-3 gap-4">
                                <Field>
                                    <FieldLabel htmlFor="data-column">Data Column</FieldLabel>
                                    <Input id="data-column" value={dataColumn} onChange={(e) => setDataColumn(e.target.value.toUpperCase())} className="w-16" placeholder="F" />
                                ...
                                <Field>
                                    ...
                                    <Input id="coa-column" value={coaColumn} onChange={(e) => setCoaColumn(e.target.value.toUpperCase())} className="w-16" placeholder="D" />
                                ...
                                <Field>
                                    ...
                                    <Input id="header-row" type="number" value={headerRow} onChange={(e) => setHeaderRow(Number(e.target.value) || 0)} className="w-20" placeholder="7" />
                                    <FieldDescription>Header {headerRow}; data starts {headerRow + 1}</FieldDescription>
                                </Field>
                            </div>
                            <div className="mt-4 grid grid-cols-2 gap-4">
                                <Field>
                                    ...
                                    <Input
                                        ...
                                        placeholder="e.g. 85"
                                    />
                                    <FieldDescription>{additionalItemsHeaderRow ? `Resumes at ${additionalItemsHeaderRow + 1}` : "Leave empty if none"}</FieldDescription>
                                </Field>
                                <Field>
                                    ...
                                    />
                                    <FieldDescription>{nonProcurementHeaderRow ? `Starts at ${nonProcurementHeaderRow + 1}` : "Leave empty if none"}</FieldDescription>
                                </Field>
                            </div>
                            <div className="mt-3 text-xs text-muted-foreground">
                                Groups: procurement [{headerRow + 1}..{additionalItemsHeaderRow ? additionalItemsHeaderRow - 1 : nonProcurementHeaderRow ? nonProcurementHeaderRow - 1 : "last"}] → additional
                                [{additionalItemsHeaderRow ? additionalItemsHeaderRow + 1 : "—"}..{nonProcurementHeaderRow ? nonProcurementHeaderRow - 1 : "last"}] → non-proc [{nonProcurementHeaderRow ? nonProcurementHeaderRow + 1 : "—"}..last]
                            </div>
                        </div>
                    ...
                    <TabsContent value="verify" className="mt-4 flex flex-col gap-4">
                        <div className="rounded-lg border p-4">
                            <p className="mb-2 text-sm font-medium">Verify procurement format (categories not in additional)</p>
                            <p className="mb-3 text-xs text-muted-foreground">Checks procurement cats (excluding additional) follow cat → coa(s) → items → cat - total (normalize only, suffix/prefix total).</p>
                                            ...
                                            {!verifyResult.valid && (
                                                <div className="mt-3 flex items-center gap-2 rounded-md border border-amber-200 bg-white p-2">
                                                    ...
                                                    <span className="text-xs">
                                                        Skip {verifyResult.errors.length} problematic row(s) and proceed to extraction (they will show as <span className="font-medium">skipped: problematic</span> in Review)
                                                    </span>
                                                </div>
                                            )}
                                        </div>
                        ...
                        <div className="flex justify-between">
                            ...
                            <Button onClick={() => setStep("extract")} disabled={!canExtract}>
                                {verifyResult?.valid ? "Next: Extract" : skipProblematic ? "Next: Extract (skipping problematic)" : "Fix verification first"}
                            </Button>
                        </div>
                    </TabsContent>
                    ...
                    <TabsContent value="extract" className="mt-4 flex flex-col gap-4">
                        {!canExtract ? (
                            <div className="rounded-lg border p-8 text-center text-sm text-muted-foreground">
                                {verifyResult ? (
                                    <>
                                        Verification failed — enable <span className="font-medium">Skip problematic rows</span> in Verify tab to proceed, or fix sheet format.
                                    </>
                                ) : (
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {extractResult && (
                                        <span className="text-sm text-muted-foreground">
                                            Raw {extractResult.filtered.length} → Unique {extractResult.unique.length} (duplicates {extractResult.duplicates.length})
                                        </span>
                                    )}
                                </div>
                                    ...
                                    <>
                                        <div className="grid grid-cols-5 gap-2 text-xs">
                                            <div className="rounded-md border p-2 text-center">
                                                <div className="text-lg font-semibold">{extractionStats?.raw}</div>
                                                <div className="text-muted-foreground">Raw</div>
                                            </div>
                                            <div className="rounded-md border p-2 text-center bg-green-50">
                                                <div className="text-lg font-semibold text-green-700">{extractionStats?.unique}</div>
                                                <div className="text-muted-foreground">Unique</div>
                                            </div>
                                            <div className="rounded-md border p-2 text-center">
                                                <div className="text-lg font-semibold">{extractResult.duplicates.length}</div>
                                                <div className="text-muted-foreground">Duplicates</div>
                                            </div>
                                            <div className="rounded-md border p-2 text-center">
                                                <div className="text-lg font-semibold">{extractResult.excludedTotal.length}</div>
                                                <div className="text-muted-foreground">Totals excluded</div>
                                            </div>
                                            <div className={`rounded-md border p-2 text-center ${extractResult.skippedProblematic.length > 0 ? "bg-amber-50 border-amber-200" : ""}`}>
                                                <div className={`text-lg font-semibold ${extractResult.skippedProblematic.length > 0 ? "text-amber-700" : ""}`}>{extractResult.skippedProblematic.length}</div>
                                                <div className="text-muted-foreground">Skipped: problematic</div>
                                            </div>
                                        </div>

                                        <div className="rounded-lg border">
                                            <div className="p-3">
                                                <h3 className="text-sm font-semibold">Review — Unique Categories ({extractResult.unique.length})</h3>
                                                <p className="text-xs text-muted-foreground">Strict dedupe by normalized (trim → collapse → lowercase). Duplicates show kept vs duplicate rows.</p>
                                            </div>
                                                    ...
                                                    <TableBody>
                                                        {extractResult.unique.slice(0, 50).map((u) => (
                                                            <TableRow key={u.normalized}>
                                                                <TableCell className="font-mono text-xs">{u.rows.join(", ")}</TableCell>
                                                                <TableCell className="text-xs">{u.raw}</TableCell>
                                                                <TableCell className="text-xs text-muted-foreground">{u.normalized}</TableCell>
                                        ...
                                        {extractResult.duplicates.length > 0 && (
                                            <div className="rounded-lg border p-3">
                                                <h4 className="text-xs font-semibold">Duplicates (normalized exact)</h4>
                                                <ul className="mt-1 max-h-32 list-disc overflow-auto pl-5 text-xs">
                                                    {extractResult.duplicates.slice(0, 10).map((d, i) => (
                                                        <li key={i}>
                                                            <span className="font-mono">{d.normalized}</span> — kept {d.keptRow}, duplicate {d.duplicateRow} (“{d.duplicateRaw}”)
                                                        </li>
                                                    ))}
                                                </ul>
                                            </div>
                                        ...
                                        {extractResult.skippedProblematic.length > 0 && (
                                            <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                                                <h4 className="text-xs font-semibold text-amber-900">Skipped: Problematic (from Verify)</h4>
                                                <p className="text-xs text-amber-800">Excluded all rows flagged in Verify (row match or normalized match). Shown for review.</p>
                                                <ul className="mt-1 max-h-32 list-disc overflow-auto pl-5 text-xs text-amber-900">
                                                    {extractResult.skippedProblematic.slice(0, 20).map((s, i) => (
                                                        <li key={i}>
                                                            <span className="font-mono">Row {s.row}:</span> “{s.raw}” → <span className="font-mono">{s.normalized}</span> — <span className="italic">{s.reason}</span>
                                                        </li>
                                                    ))}
                                                </ul>
                                        ...
                                        <div className="flex items-center gap-2 rounded-lg border p-3">
                                            <Button onClick={handleImport} disabled={importing || extractResult.unique.length === 0}>
                                                {importing ? (
                                                    <>
                                                        <Spinner /> Importing...
                                                    </>
                                            ...
                                            <span className="text-xs text-muted-foreground">Will create ppmp_categories where not exists (strict normalized match).</span>
                                        </div>
                                    </>
                                )}
                            </>
