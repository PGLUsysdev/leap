...
export const formatQty = (val?: number) => (val && val > 0 ? val.toString() : "");
...
export const getPpmpColumnDefs = <T extends Record<string, any>>(): ColumnDef<T>[] => [
    ...
    },
    {
        ...
        cell: (item) => (
            <Text style={cellStyle("center")}>
                {item.ppmp_price_list?.unit_of_measurement ?? item.unit_of_measurement ?? "-"}
            </Text>
        ),
    },
    {
    ...
    },
    {
        ...
        cell: (item) => {
            if (item.total_qty !== undefined) {
                return <Text style={cellStyle("center")}>{formatQty(item.total_qty)}</Text>;
            }

            const total = MONTHS.reduce((sum, m) => sum + (Number(item[`${m.key}_qty`]) || 0), 0);
    ...
    },
    {
        ...
        cell: (item) => {
            if (item.total_amount !== undefined) {
                return <Text style={cellStyle("right")}>{formatCurrency(item.total_amount)}</Text>;
            }
            ...
            return <Text style={cellStyle("right")}>{formatCurrency(total.toString())}</Text>;
        },
    },
    ...
    ...MONTHS.flatMap(({ key, label }) => [
        {
            ...
            cell: (item: T) => (
                <Text style={cellStyle("center")}>{formatQty(item[`${key}_qty`])}</Text>
            ),
        },
        {
            id: `${key}_amount`,
            ...
            width: AMT_COL_WIDTH,
            cell: (item: T) => (
                <Text style={cellStyle("right")}>{formatCurrency(item[`${key}_amount`])}</Text>
            ),
        },
    ]),
];
