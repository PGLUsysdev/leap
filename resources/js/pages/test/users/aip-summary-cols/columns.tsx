...
const columns = [
    ...
    columnHelper.accessor("name", {
        ...
        size: 400,
        cell: ({ row, getValue }) => (
            <div style={{ paddingLeft: `${row.original.depth * 20}px` }} className="flex gap-2">
    ...
    columnHelper.group({
        ...
        columns: [
            columnHelper.accessor("aip_entries", {
                ...
                cell: (info) =>
                    info.getValue()?.[0] ? formatDate(info.getValue()[0].start_date) : "—",
                meta: { rowSpan: true },
            }),
            columnHelper.accessor("aip_entries", {
                id: "end_date",
                ...
                cell: (info) =>
                    info.getValue()?.[0] ? formatDate(info.getValue()[0].end_date) : "—",
                meta: { rowSpan: true },
            }),
        ],
    }),
    columnHelper.accessor("aip_entries", {
        ...
        size: 400,
        cell: (info) => (
            <div className="text-wrap">{info.getValue()?.[0]?.expected_output || "—"}</div>
        ),
        meta: { rowSpan: true },
    }),
        ...
        columns: [
            ...
            columnHelper.display({
                ...
                cell: ({ row }) => {
                    ...
                    return (
                        <div className="text-right font-bold">{formatNumber(total.toString())}</div>
                    );
                },
            }),
        ],
    ...
    columnHelper.display({
        ...
        cell: ({ row, table }) => (
            <div className="flex items-center gap-1">
                ...
                <Button size="icon" onClick={() => table.options.meta?.onEdit?.(row.original)}>
