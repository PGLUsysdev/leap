...
const cellCenter = { margin: 2, fontSize: 6.5, paddingVertical: 1, textAlign: "center" as const };
...
const quarterDefs = ([1, 2, 3, 4] as const).flatMap<ColumnDef<AppItem>>((q) => [
    {
        ...
        cell: (item) => <Text style={cellCenter}>{item[`q${q}_qty`] || "-"}</Text>,
    },
    {
        id: `q${q}_amount`,
        width: APP_COLUMN_WIDTHS[5 + q * 2],
        cell: (item) => <Text style={cellRight}>{formatAppNumber(item[`q${q}_amount`])}</Text>,
    },
]);
    ...
    return [
        {
            ...
            cell: (item) => <Text style={cellCenter}>{item.ppmp_price_list?.item_number}</Text>,
        },
        {
            id: "description",
            width: APP_COLUMN_WIDTHS[1],
            cell: (item) => <Text style={cellLeft}>{item.ppmp_price_list?.description}</Text>,
        },
        {
            id: "uom",
            width: APP_COLUMN_WIDTHS[2],
            cell: (item) => (
                <Text style={cellCenter}>{item.ppmp_price_list?.unit_of_measurement}</Text>
            ),
        },
        {
            id: "price",
            ...
            cell: (item) => (
                <Text style={cellRight}>{formatAppNumber(item.ppmp_price_list?.price)}</Text>
            ),
        },
        {
            id: "qty",
        ...
        },
        {
            ...
            cell: (item) => <Text style={cellRight}>{formatAppNumber(item.total_amount)}</Text>,
        },
        ...quarterDefs,
    ];
}
