...
const rendererLoaders: {
    ...
    "aip-summary": () => import("@/pages/aip-summary/pdf-render/render-aip-summary-pdf"),
    "fs-summary": () =>
 ...
 */
export async function renderPdf(kind: PdfKind, payload: unknown): Promise<Blob> {
    switch (kind) {
        case "app":
            return (await rendererLoaders.app()).renderPdf(payload as AppPdfPayload);
        case "aip-summary":
        ...
        case "ppmp":
            return (await rendererLoaders.ppmp()).renderPdf(payload as PpmpPdfPayload);
        case "ppmp-coa-summary":
            return (await rendererLoaders["ppmp-coa-summary"]()).renderPdf(
                payload as PpmpCoaSummaryPdfPayload,
            );
        default:
            throw new Error(`Unknown PDF renderer kind: ${kind satisfies never}`);
    }
}
