...
const coaA: ExistingCoa = { id: 1, account_number: "50203010", path: "5-02-03-010", account_title: "Drugs and Medicines Expenses" };
const coaB: ExistingCoa = { id: 2, account_number: "50201010", path: "5-02-01-010", account_title: "Traveling Expenses" };

function row(overrides: Partial<BatchMatchableRow> & { key: string }): BatchMatchableRow {
    return {
...
describe("groupUnmatchedByExtractedCoa", () => {
    it("should_MergeRows_When_SameNormalizedLabel", () => {
        ...
        const rows = [
            ...
            row({ key: "c", coa: "Traveling", coaNorm: "traveling", coaTopMatches: [{ coa: coaB, score: 99 }] }),
        ];
        // Act
    ...
    it("should_HaveNullTopSuggestion_When_NoMatches", () => {
        const groups = groupUnmatchedByExtractedCoa([row({ key: "a", coaTopMatches: [] })]);
    ...
    it("should_SortByCountDesc_When_MultipleGroups", () => {
        // Arrange
        const rows = [
            row({ key: "a", coaNorm: "traveling", coa: "Traveling", coaTopMatches: [{ coa: coaB, score: 99 }] }),
            row({ key: "b" }),
            row({ key: "c" }),
        ];
        // Act
        const groups = groupUnmatchedByExtractedCoa(rows);
        // Assert
        expect(groups.map((g) => g.coaNorm)).toEqual(["drugs and medicines", "traveling"]);
    });

    it("should_ReturnEmpty_When_NoRowsNeedMatching", () => {
        expect(groupUnmatchedByExtractedCoa([])).toEqual([]);
        expect(groupUnmatchedByExtractedCoa([row({ key: "a", coaExists: true })])).toEqual([]);
    });
});
