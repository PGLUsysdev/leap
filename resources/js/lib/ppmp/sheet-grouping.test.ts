...
const CFG = {
    columnConfig: { category: 'F', coa: 'D' },
    coaLabelMode: 'without-label' as const,
};
const CFG_WITH_LABEL = {
    columnConfig: { category: 'F', coa: 'D' },
    coaLabelMode: 'with-label' as const,
};
...
function makeSheet(cells: Record<string, string>): ExcelJS.Worksheet {
    ...
    const rows = Object.keys(cells).map((addr) =>
        Number(addr.replace(/^[A-Z]+/, '')),
    );
...
function withoutLabelOpts(
    overrides: Partial<SheetGroupOptions> = {},
): SheetGroupOptions {
    return {
...
describe('groupSheet zero/one/many', () => {
    ...
    it('should_GroupSingleCategoryCoaAndItems_When_WithoutLabelHappyPath', () => {
        ...
        expect(groups[0].coas[0]).toMatchObject({
            coa: 'COA-1',
            coaRow: 3,
            items: 2,
        });
...
describe('groupSheet boundaries and errors', () => {
    it('should_ReportMissingTotal_When_CategoryNeverClosed', () => {
        ...
        expect(
            errors.some((e) =>
                e.message.includes('missing closing "Office Supplies - TOTAL"'),
            ),
        ).toBe(true);
    });
    ...
    it('should_ReportTotalMismatch_When_TotalNameDoesNotMatchStrict', () => {
        ...
        const groups = groupSheet(
            ...
            2,
            4,
            withoutLabelOpts({ strictTotal: true }),
            onError,
        );
        // Assert
        expect(groups).toHaveLength(1);
        expect(errors.some((e) => e.message.includes('Total mismatch'))).toBe(
            true,
        );
    });
...
describe('groupSheet with-label mode', () => {
    ...
    it('should_ImplicitCreateCoa_When_MissingCoaModeIsImplicitCreate', () => {
        ...
        const groups = groupSheet(
            ...
            2,
            4,
            withLabelOpts({ missingCoaMode: 'implicitCreate' }),
            onError,
        );
        // Assert
        expect(errors).toEqual([]);
    ...
    it('should_ReportItemWithoutCoa_When_WithLabelAndMissingCoaIsError', () => {
        ...
        const groups = groupSheet(
            ...
            2,
            4,
            withLabelOpts({ missingCoaMode: 'error' }),
            onError,
        );
        // Assert
        expect(
            errors.some((e) => e.message.includes('without active COA')),
        ).toBe(true);
        expect(groups).toHaveLength(1);
