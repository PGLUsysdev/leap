...
describe("columnToNumber / numberToColumn / leftColumn", () => {
    ...
    it.each([
        ...
        ["F", "G"],
        ["Z", "AA"],
    ])("should_Return%s_When_TakingLeftOf%s", (expected: string, col: string) => {
...
describe("getCategoryMatch", () => {
    const existing: ExistingCategory[] = [
        ...
        { id: 2, name: "Oil Products", is_non_procurement: false, is_additional: false },
        { id: 3, name: "Laboratory Equipment", is_non_procurement: false, is_additional: false },
    ];
    ...
    it("should_ReturnNone_When_NoCloseMatch", () => {
        const result = getCategoryMatch(normalize("unrelated category xyz"), existing);
        expect(result.type).toBe("none");
    });
    ...
    it("should_LimitTopMatchesToThree_When_ManyPartials", () => {
        ...
        const result = getCategoryMatch(normalize("Office Supplies Branch X"), many);
        expect(result.type).toBe("partial");
...
describe("getCoaMatch", () => {
    const coas: ExistingCoa[] = [
        { id: 1, account_number: "50203010", path: "5-02-03-010", account_title: "Office Supplies Expenses" },
        { id: 2, account_number: "50201010", path: "5-02-01-010", account_title: "Traveling Expenses" },
    ];

    it("should_ReturnStrictByTitle_When_ModeIsAutoAndTitleMatches", () => {
        const result = getCoaMatch(normalize("Office Supplies Expenses"), coas, "auto");
        expect(result.type).toBe("strict");
        expect(result.match?.id).toBe(1);
    });
    ...
    it("should_NotMatchTitle_When_ModeIsAccountNumber", () => {
        const result = getCoaMatch(normalize("Office Supplies Expenses"), coas, "account_number");
        expect(result.type).not.toBe("strict");
    });

    it("should_NotMatchNumber_When_ModeIsAccountTitle", () => {
        const result = getCoaMatch(normalize("50203010"), coas, "account_title");
        expect(result.type).not.toBe("strict");
    });
