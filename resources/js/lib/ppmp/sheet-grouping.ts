...
export type CatGroup = { cat: string; catRow: number; coas: Array<{ coa: string; coaRow: number; items: number }>; totalRow?: number };
...
export function isCoaLabel(
    ...
    dataColumn: string,
    coaColumn: string,
): { isCoaLabel: boolean; nextCoaRaw: string | null; nextCoaNorm: string | null } {
...
export function groupSheet(
    ws: ExcelJS.Worksheet,
    cfg: { columnConfig: { category: string; coa: string }; coaLabelMode: "with-label" | "without-label" },
    startRow: number,
    endRow: number,
    opts: SheetGroupOptions,
    onError: (row: number, message: string) => void,
): CatGroup[] {
    const { sectionName, coaLabelMode, createSentinelIfMissing = false, strictTotal = true, missingCoaMode = "error", missingCategoryMode = "error" } = opts;
    ...
    for (let r = startRow; r <= endRow && r <= lastRow; r++) {
        ...
        if (coaNorm && dataRaw) {
            if (!currentCat) {
                if (createSentinelIfMissing && (sectionName === "additional" || sectionName === "non-procurement")) {
                    ...
                    if (missingCategoryMode === "error") onError(r, `Item at row ${r} ("${dataRaw}") found without active category in ${sectionName}`);
                    continue;
                }
            }
            if (coaLabelMode === "without-label") {
                if (!currentCoa || coaNorm !== normalize(currentCoa.coa)) {
                    if (currentCoa) {
                        if (currentCoa.items === 0) onError(currentCoa.coaRow, `COA "${currentCoa.coa}" at row ${currentCoa.coaRow} in cat "${currentCat.cat}" has no items before next COA (${sectionName})`);
                        currentCat.coas.push(currentCoa);
                    }
                ...
                if (!currentCoa) {
                    if (missingCoaMode === "implicitCreate") {
                        ...
                        onError(r, `Item at row ${r} ("${dataRaw}") found without active COA in cat "${currentCat.cat}" (${sectionName})`);
                        continue;
                    }
                }
                if (coaNorm !== normalize(currentCoa.coa)) {
                    if (strictTotal) onError(r, `Item COA mismatch at row ${r}: D="${coaRaw}" != current COA "${currentCoa.coa}" in cat "${currentCat.cat}" (${sectionName})`);
                    currentCat.coas.push(currentCoa);
                    currentCoa = { coa: coaRaw!, coaRow: r, items: 1 };
                } else {
                    currentCoa.items += 1;
        ...
        if (isTotalRow(dataNorm)) {
            ...
            if (!currentCat) {
                onError(r, `Total "${dataRaw}" at row ${r} without active category (${sectionName})`);
            } else if (strictTotal && expected && dataNorm !== expected) {
                onError(r, `Total mismatch at row ${r}: got "${dataRaw}" (norm "${dataNorm}") expected "${currentCat.cat} - TOTAL" (${sectionName})`);
            }
            if (currentCat) {
                if (currentCoa) {
                    currentCat.coas.push(currentCoa);
                ...
                if (currentCat.coas.length === 0) {
                    onError(r, `Category "${currentCat.cat}" at row ${currentCat.catRow} has no COA groups before total (${sectionName})`);
                } else {
                    for (const c of currentCat.coas) if (c.items === 0) onError(c.coaRow, `COA "${c.coa}" at row ${c.coaRow} in cat "${currentCat.cat}" has no items (${sectionName})`);
                }
                flushCat(r);
            }
            continue;
            ...
            if (isLabel) {
                if (!currentCat) {
                    if (createSentinelIfMissing && (sectionName === "additional" || sectionName === "non-procurement")) {
                        ...
                        onError(r, `COA "${dataRaw}" at row ${r} found without active category (${sectionName})`);
                        continue;
                    }
                }
                if (currentCoa) {
        ...
        if (currentCat) {
            onError(r, `Category "${dataRaw}" at row ${r} started before previous cat "${currentCat.cat}" (row ${currentCat.catRow}) closed with " - TOTAL" (${sectionName})`);
            if (currentCoa) {
                if (currentCoa.items === 0) onError(currentCoa.coaRow, `COA "${currentCoa.coa}" at row ${currentCoa.coaRow} in cat "${currentCat.cat}" has no items (${sectionName})`);
                currentCat.coas.push(currentCoa);
                currentCoa = null;
            }
            catGroups.push(currentCat);
    ...
    if (currentCat) {
        if (currentCoa) {
            if (currentCoa.items === 0) onError(currentCoa.coaRow, `COA "${currentCoa.coa}" at row ${currentCoa.coaRow} in cat "${currentCat.cat}" has no items at end (${sectionName})`);
            currentCat.coas.push(currentCoa);
        }
        if (!currentCat.totalRow) onError(currentCat.catRow, `Category "${currentCat.cat}" at row ${currentCat.catRow} missing closing "${currentCat.cat} - TOTAL" (${sectionName}) (found ${currentCat.coas.length} COA(s))`);
        else if (currentCat.coas.length === 0) onError(currentCat.catRow, `Category "${currentCat.cat}" has no COAs (${sectionName})`);
        catGroups.push(currentCat);
    }
